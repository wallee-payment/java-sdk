 ...
 */
@JsonPropertyOrder({
  TransactionInvoiceCommentCreate.JSON_PROPERTY_CONTENT,
  TransactionInvoiceCommentCreate.JSON_PROPERTY_TRANSACTION_INVOICE
})
@JsonTypeName("TransactionInvoiceComment.Create")
...
public class TransactionInvoiceCommentCreate {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(content, transactionInvoice);
  }
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getTransactionInvoice() != null) {
      try {
        joiner.add(String.format("%stransactionInvoice%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getTransactionInvoice()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
