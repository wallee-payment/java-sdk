 ...
 */
@JsonPropertyOrder({
  BankAccountType.JSON_PROPERTY_IDENTIFIER_NAME,
  BankAccountType.JSON_PROPERTY_NAME,
  BankAccountType.JSON_PROPERTY_DESCRIPTION,
  BankAccountType.JSON_PROPERTY_ID
})
...
public class BankAccountType {
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(this.identifierName, bankAccountType.identifierName) &&
        Objects.equals(this.name, bankAccountType.name) &&
        Objects.equals(this.description, bankAccountType.description) &&
        Objects.equals(this.id, bankAccountType.id);
  }

  @Override
  public int hashCode() {
    return Objects.hash(identifierName, name, description, id);
  }
  ...
  public String toUrlQueryString(String prefix) {
    ...
    // add `identifierName` to the URL query string
    if (getIdentifierName() != null) {
      for (String _key : getIdentifierName().keySet()) {
        try {
          joiner.add(String.format("%sidentifierName%s%s=%s", prefix, suffix,
              "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, _key, containerSuffix),
              getIdentifierName().get(_key), URLEncoder.encode(String.valueOf(getIdentifierName().get(_key)), "UTF-8").replaceAll("\\+", "%20")));
