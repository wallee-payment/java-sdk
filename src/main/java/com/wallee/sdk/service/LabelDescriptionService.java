...
public class LabelDescriptionService {
  ...
  /**
    ...
    * @see <a href="https://app-wallee.com/doc/api/web-service/v1#label-description-service--all">All Documentation</a>

    **/
    public List<LabelDescriptor> all() throws IOException {
        HttpResponse response = allForHttpResponse();
        String returnType = "List&lt;LabelDescriptor&gt;";
        if(returnType.equals("String")){
          return (List<LabelDescriptor>) (Object) response.parseAsString();
        }
        TypeReference typeRef = new TypeReference<List<LabelDescriptor>>() {};
        if (isNoBodyResponse(response)) {
            throw new WalleeSdkException(ErrorCode.ENTITY_NOT_FOUND, "Entity was not found for: " + typeRef.getType().getTypeName());
        }
        return (List<LabelDescriptor>)apiClient.getObjectMapper().readValue(response.getContent(), typeRef);
    }
  ...
  /**
    ...
    **/
    ...
    public HttpResponse allForHttpResponse() throws IOException {
        URIBuilder uriBuilder = URIBuilderUtil.create(apiClient.getBasePath() + "/label-description-service/all");
  ...
  /**
    ...
    * @throws IOException if an error occurs while attempting to invoke the API
    * For more information visit this link.
    * @see <a href="https://app-wallee.com/doc/api/web-service/v1#label-description-service--read">Read Documentation</a>

    **/
    public LabelDescriptor read(Long id) throws IOException {
        HttpResponse response = readForHttpResponse(id);
        String returnType = "LabelDescriptor";
        if(returnType.equals("String")){
          return (LabelDescriptor) (Object) response.parseAsString();
        }
        TypeReference typeRef = new TypeReference<LabelDescriptor>() {};
        if (isNoBodyResponse(response)) {
            throw new WalleeSdkException(ErrorCode.ENTITY_NOT_FOUND, "Entity was not found for: " + typeRef.getType().getTypeName());
        }
        return (LabelDescriptor)apiClient.getObjectMapper().readValue(response.getContent(), typeRef);
    }
  ...
  /**
    ...
    **/
    ...
    public HttpResponse readForHttpResponse(Long id) throws IOException {
        ...
        URIBuilder uriBuilder = URIBuilderUtil.create(apiClient.getBasePath() + "/label-description-service/read");
        if (id != null) {
            String key = "id";
            Object value = id;
            uriBuilder = URIBuilderUtil.applyQueryParam(uriBuilder, key, value);
